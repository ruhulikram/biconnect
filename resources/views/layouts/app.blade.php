...
<head>
    ...
    <meta name="description"
        content="@yield('meta_description', 'BiConnect adalah platform kolaborasi project dan diskusi untuk mahasiswa Universitas Bina Sarana Informatika.')">
    ...
    @if(session('success'))
        <div x-data="{ show: true }" x-show="show" x-init="setTimeout(() => show = false, 4500)"
            ...
            <div class="flex-1 min-w-0">
                <p class="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest leading-none">
                    SUKSES</p>
                <p class="text-sm text-slate-700 dark:text-slate-200 mt-1.5 leading-relaxed">{{ session('success') }}</p>
            </div>
            <button @click="show = false"
                class="text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 shrink-0 mt-0.5">
