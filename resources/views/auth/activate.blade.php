...
@section('content')
    <div class="space-y-6" x-data="{
        ...
        <a href="{{ url('/') }}" class="flex justify-center">
            <img src="{{ asset('images/biconnect-logo.webp') }}" alt="BiConnect" width="160" height="40"
                class="h-10 w-auto">
        </a>
        ...
        <form action="{{ route('auth.send-otp') }}" method="POST" class="space-y-4">
            ...
            <div class="space-y-1.5">
                ...
                <div class="relative">
                    ...
                    <input id="email" name="email" type="email" x-model="email" placeholder="nama@bsi.ac.id" required class="w-full h-12 rounded-input border bg-white pl-10 pr-4 py-3 text-sm placeholder-gray-400 text-gray-900 transition-shadow duration-150 focus:outline-none focus:border-primary focus:shadow-focus
                    ...
                    <span class="absolute right-3 top-1/2 -translate-y-1/2" x-show="email.length > 0" x-cloak>
                        <svg x-show="isValidDomain" class="w-4.5 h-4.5 text-green-500" fill="currentColor"
                            viewBox="0 0 20 20">
                            <path fill-rule="evenodd"
                                d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.857-9.809a.75.75 0 00-1.214-.882l-3.483 4.79-1.88-1.88a.75.75 0 10-1.06 1.061l2.5 2.5a.75.75 0 001.137-.089l4-5.5z"
                                clip-rule="evenodd" />
                        </svg>
                        <svg x-show="!isValidDomain" class="w-4.5 h-4.5 text-red-400" fill="currentColor"
                            viewBox="0 0 20 20">
                            <path fill-rule="evenodd"
            ...
            {{-- Submit Button --}}
            <button type="submit" :disabled="!isValidDomain" class="w-full h-12 rounded-input bg-primary text-white text-sm font-medium transition-colors duration-150
